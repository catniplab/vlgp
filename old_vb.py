...
def variational(spike, p, prior_mean, prior_var, prior_w,
    ...
    if K0 is None:
        ...
        for l in range(L):
            post_inv[l, :, :] = linalg.inv(post_cov[l, :, :])
    ...
    while not convergent and it < maxiter:
        if not fixb:
            for n in range(N):
                ...
                nhess_b = np.dot(regressor.T, (regressor.T * rate[:, n]).T)
                if np.linalg.norm(grad_b, ord=np.inf) < eps:
                    break
                try:
                    delta_b = rb[n] * linalg.solve(nhess_b, grad_b, sym_pos=True)
                except linalg.LinAlgError as e:
                    print('beta', e)
                    continue
                last_b[:, n] = beta[:, n]
                last_rate[:, n] = rate[:, n]
                predict = np.inner(grad_b, delta_b) - 0.5 * np.dot(delta_b, np.dot(nhess_b, delta_b))
                beta[:, n] += delta_b
        ...
        if not fixa:
            for l in range(L):
                ...
                nhess_a = np.diag(np.dot(rate.T, post_mean[:, l] * post_mean[:, l])
                ...
                if constrain_a == 'lag':
                    ...
                    hess_a_lag[:N, :N] = -nhess_a + 2 * lam_a[l] * eyeN
                    hess_a_lag[N:, :N] = 2 * alpha[l, :]
                    hess_a_lag[:N, N:] = hess_a_lag[N:, :N].T
                    hess_a_lag[N:, N:] = 0
                    if linalg.norm(grad_a_lag, ord=np.inf) < eps:
                        break
                    try:
                        delta_a_lag = -ra[l] * linalg.solve(-hess_a_lag, grad_a_lag, sym_pos=True)
                    ...
                    if np.linalg.norm(grad_a, ord=np.inf) < eps:
                        break
                    try:
                        delta_a = ra[l] * linalg.solve(nhess_a, grad_a, sym_pos=True)
                    except np.linalg.LinAlgError as e:
                        print('alpha', e)
                        continue
                    last_a[l, :] = alpha[l, :]
                    ...
                    if np.linalg.norm(alpha[l, :]) > 0:
                        predict = np.inner(grad_a, delta_a / linalg.norm(alpha[l, :]) * anorm) \
                                  - 0.5 * np.dot(delta_a / linalg.norm(alpha[l, :]) * anorm,
                                                 np.dot(nhess_a, delta_a / linalg.norm(alpha[l, :]) * anorm))
                        alpha[l, :] /= linalg.norm(alpha[l, :]) / anorm
                    else:
                        predict = np.inner(grad_a, delta_a) - 0.5 * np.dot(delta_a, np.dot(nhess_a, delta_a))
        ...
        if not fixm:
            for l in range(L):
                ...
                nhess_m = np.diag(np.dot(rate, alpha[l, :] * alpha[l, :])) + prior_inv[l, :, :]
                if constrain_m == 'lag':
                    grad_m_lag[:T] = grad_m + lam_m[l]
                    grad_m_lag[T:] = np.sum(post_mean[:, l])
                    hess_m_lag[:T, :T] = -nhess_m
                    ...
                    if np.linalg.norm(grad_m, ord=np.inf) < eps:
                        break
                    try:
                        delta_m = rm[l] * linalg.solve(nhess_m, grad_m, sym_pos=True)
                    except np.linalg.LinAlgError as e:
                        print('post_mean', e)
                        continue
                    last_m[:, l] = post_mean[:, l]
                    last_rate[:] = rate
                    predict = np.inner(grad_m, delta_m) - 0.5 * np.dot(delta_m, np.dot(nhess_m, delta_m))
                    post_mean[:, l] += delta_m
                    updaterate(range(T), range(N))
                    lb = lowerbound(spike, beta, alpha, prior_mean, prior_inv, post_mean, post_cov, regressor=regressor, frate=rate)
                    if np.isnan(lb) or lb < lbound[it - 1]:
        ...
        if hyper:
            for l in range(L):
                vl = np.trace(linalg.solve(np.dot(prior_chol[l, :, :], prior_chol[l, :, :].T),
                                           np.outer(post_mean - prior_mean, post_mean - prior_mean) + post_cov[l, :, :],
                                           sym_pos=True)) / T
